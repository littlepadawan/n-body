...
# Put data of last time step in the same order as input
data = np.ravel(data)
for i in range(0, N*6, 6):
    data[i]   = (x_pos_history[NUMOFCALC-1][i//6])
    data[i+1] = (y_pos_history[NUMOFCALC-1][i//6])
    data[i+2] = (mass[i//6])
    data[i+3] = (x_vel_history[NUMOFCALC-1][i//6])
    data[i+4] = (y_vel_history[NUMOFCALC-1][i//6])
    data[i+5] = (brightness[i//6])


# Write data for last time step to file
data.tofile("output.gal")
